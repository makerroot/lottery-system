  ...
  <div class="lottery-page" :style="{ '--company-color': themeColor }">
    ...
    >
      <div style="text-align: center; padding: 20px 0;">
        <div v-if="!registerQRCode" style="padding: 40px 0;">
          <a-spin size="large" />
          <p style="margin-top: 16px; color: #666;">加载二维码中...</p>
        </div>
        <div v-else>
          <img
            :src="registerQRCode"
            alt="注册二维码"
            style="width: 100%; max-width: 280px; border: 4px solid #667eea; border-radius: 12px; padding: 8px; background: white;"
          />
          <div style="margin-top: 24px; padding: 20px; background: rgba(102, 126, 234, 0.05); border-radius: 12px; border: 1px solid rgba(102, 126, 234, 0.2);">
            <p style="margin: 12px 0; font-size: 14px; color: #333;">📱 打开手机相机或微信扫一扫</p>
            <p style="margin: 12px 0; font-size: 14px; color: #333;">🔍 扫描上方二维码</p>
            <p style="margin: 12px 0; font-size: 14px; color: #333;">✍️ 填写信息完成注册</p>
            <p style="margin: 12px 0; font-size: 12px; color: #666;">注册成功后将自动加入抽奖池</p>
          </div>
        </div>
      </div>
    </a-modal>
        ...
        <div class="config-card glass control-panel">
          ...
          <div class="config-form">
            ...
            <div class="refresh-section">
              <a-button
                @click="() => { fetchData(); message.success('数据已刷新') }"
                size="large"
                block
                style="background: linear-gradient(135deg, #52c41a 0%, #73d13d 100%); border: none; color: white; height: 40px; font-size: 14px; font-weight: bold; margin-bottom: 12px;"
              >
                <SyncOutlined :spin="false" /> 🔄 刷新数据
              </a-button>
              <p style="text-align: center; color: #999; font-size: 11px; margin-top: -8px; margin-bottom: 12px;">
                数据每 5 秒自动刷新
              </p>
            </div>
            ...
            <div class="qr-section">
              ...
              >
                <QrcodeOutlined /> 📱 扫码参与抽奖
              </a-button>
              <p style="text-align: center; color: #999; font-size: 12px; margin-top: -8px; margin-bottom: 16px;">
                点击查看二维码，用户扫码即可注册
              </p>
            </div>
...
import { QrcodeOutlined, SyncOutlined } from '@ant-design/icons-vue'
...
const DATA_REFRESH_INTERVAL = 5000 // 5秒刷新一次
...
const handleDraw = async () => {
  ...
  try {
    ...
    if (!stats.undrawn_users || stats.undrawn_users === 0) {
      message.warning('所有用户都已抽过奖')
      return
    }

    if (stats.undrawn_users < drawCount.value) {
      message.warning(`未抽奖用户不足${drawCount.value}人，当前只有${stats.undrawn_users}人`)
      return
    }
  } catch (error) {
...
onMounted(async () => {
  // 从路由获取公司代码（支持 hash 模式）
  const urlCompanyCode = route.query.company

  // 如果 URL 中有 company 参数，设置 currentCompanyCode
